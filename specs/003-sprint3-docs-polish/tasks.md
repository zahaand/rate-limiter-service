...
- [X] T001 Add `val swaggerUiVersion = "5.7.0"` version variable and `implementation("io.github.smiley4:ktor-swagger-ui:$swaggerUiVersion")` to the dependencies block in `build.gradle.kts`; run `./gradlew dependencies | grep swagger` to confirm no transitive conflicts
