...
`/check` three times and confirm each response carries `allowed: true` with a decrementing
...
1. **Given** a policy of 3 requests per 10 seconds is configured for key `"user-123"`,
   **When** `POST /check {"key": "user-123"}` is called three times within the window,
...
- `POST /check` with a key that contains only whitespace characters must be treated as blank
  and rejected with HTTP 400.
- `POST /limits` with `limit = 0` is valid (all requests for that key will be rejected).
- `POST /limits` with `windowSeconds = 0` must be rejected with HTTP 400.
...
- **FR-001**: The system MUST expose a `POST /check` endpoint that accepts a client key,
...
- **FR-002**: The check endpoint MUST always respond with HTTP 200 regardless of whether the
  request is allowed or rejected. HTTP 429 is explicitly out of scope.
- **FR-003**: The check endpoint MUST reject requests where `key` is blank (empty or
  whitespace-only) with HTTP 400 and body `{"error": "key must not be blank"}`.
...
- **FR-005**: Every REJECTED decision MUST be logged with: the client key, strategy, limit,
  window duration, and the timestamp of the decision.
- **FR-006**: The system MUST expose a `POST /limits` endpoint that creates or replaces a
  per-key policy and returns HTTP 201 with the stored policy echoed back.
- **FR-007**: The system MUST validate all `POST /limits` fields:
...
- **FR-008**: The system MUST expose a `GET /limits/{key}` endpoint that returns the stored
  policy for the key (HTTP 200) or HTTP 404 with `{"error": "policy not found for key: {key}"}`.
- **FR-009**: The system MUST expose a `DELETE /limits/{key}` endpoint that removes the
...
- **FR-011**: The health endpoint MUST return HTTP 200 with `{"status":"UP","redis":"UP"}`
  ...
  is unreachable. No other status combinations are defined.
...
- **Rate Check Request**: Inbound payload for `/check` — a single non-blank client key
...
- **Limit Config Request**: Inbound payload for `POST /limits` — a client key, numeric limit
...
- **SC-002**: `POST /check` correctly returns `allowed: false` after the configured limit is
  exhausted in 100% of test runs, including concurrent calls at the boundary.
- **SC-003**: `POST /check` uses the service-wide default policy in 100% of cases where no
  per-key policy is stored — no error is surfaced, no fallback exception escapes.
- **SC-004**: All four CRUD operations on `/limits` are consistent: a policy stored via POST
  is retrievable via GET with identical values, and is absent after DELETE.
...
- Policy creation via `POST /limits` is an upsert — if a policy for the key already exists,
