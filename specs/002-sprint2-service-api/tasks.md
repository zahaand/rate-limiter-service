...
- [ ] T024 Add `healthRoute(commands)` to the bare (non-`/v1`) routing block and add `monitor.subscribe(ApplicationStopped) { connection.close(); redisClient.shutdown() }` graceful shutdown hook (use `monitor` directly on `Application`, not the deprecated `environment.monitor` — I2); Netty engine shutdown grace period is already configured in T008 (`shutdownGracePeriod = 5_000L`, `shutdownTimeout = 5_000L`) and satisfies the FR-013 5-second in-flight wait; **SC-007 note**: shutdown resource-release verification is delegated to T025 (G3) in `src/main/kotlin/dev/zahaand/ratelimiter/Application.kt`
